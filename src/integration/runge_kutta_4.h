...
class RungeKutta4 {
 private:
  ...
  T ForwardIntegrateStep(const std::function<T(double, const T&)>& f,
                         const T& y0, const TimeSpan& ts) const {
    ...
    constexpr double c1 = (1.0 / 6.0), c2 = (1.0 / 3.0), c3 = (1.0 / 3.0),
                     c4 = (1.0 / 6.0);
 ...
 public:
  ...
  T ForwardIntegrate(const std::function<T(double, const T&)>& f, const T& y0,
