...
namespace game_engine {
    ...
    TrajectoryCode TrajectoryWatchdogStatus::Read(const std::string& quad_name) const {
      std::lock_guard<std::mutex> lock(mtx_);
      try {
    ...
    void TrajectoryWatchdogStatus::Write(const std::string& quad_name, const TrajectoryCode infraction_occurs) {
      std::lock_guard<std::mutex> lock(mtx_);
      try {
