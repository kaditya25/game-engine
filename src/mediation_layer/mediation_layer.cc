...
//          }
    ...
    Polyhedron MediationLayer::ExpandQuad(const Eigen::Vector3d cm, const double inflation_distance) {
    ...
    void MediationLayer::TransferData(
      ...
      while(this->ok_) {
        ...
        // Determine if quad has violated state constraints. If it has, freeze it in place
        ...
        if((quad_state_watchdog_status->Read(key)).code == MediationLayerCode::QuadViolatesMapBoundaries) {
...
//          std::cout << key << " Quad too close to another quad." << std::endl;
        ...
        } else if((trajectory_watchdog_status->Read(key)).code == MediationLayerCode::QuadTrajectoryCollidesWithAnotherQuad) {
