...
namespace game_engine {
  ...
  void MediationLayer::
  TransferData(const std::string& key,
               const Map3D& map,
    ...
    while(true == this->ok_) {
      ...
      if(true == quad_state_watchdog_status->Read(key)) {
        std::cout << key << " has flown too close to obstacle or boundary; freezing."
                  << std::endl;

        QuadState current_quad_state;
        quad_state_warden->Read(key, current_quad_state);

        static const Eigen::Vector3d freeze_quad_position =
          current_quad_state.Position();
