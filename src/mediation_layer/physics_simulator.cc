...
Eigen::VectorXd mvnrnd(const Eigen::VectorXd& mu, const Eigen::MatrixXd& sigma,
                       std::mt19937& gen) {
  // Normal distribution
  ...
  return mu + sqrt_sigma *
                  Eigen::MatrixXd(mu.rows(), mu.cols()).unaryExpr([&](auto x) {
                    return dist(gen);
                  });
}
...
Eigen::VectorXd VonKarman(const Eigen::VectorXd& Omega, const double& sigma_u,
                          const double& L_u) {
  return (2 * L_u * std::pow(sigma_u, 2) / M_PI) *
...
GenerateWindAccelerationVector(const double max_time,
                               const double max_frequency, const double sigma_u,
                               const double L_u, const double V,
                               std::mt19937& gen) {
  // Number of samples
  ...
  noise_freq.resize(N);
  {
    ...
    for (size_t idx = 0; idx < N; ++idx) {
      ...
      in[idx][0] = mvnrnd(Eigen::Matrix<double, 1, 1>(0),
                          Eigen::Matrix<double, 1, 1>::Identity(), gen)(0);
      in[idx][1] = mvnrnd(Eigen::Matrix<double, 1, 1>(0),
                          Eigen::Matrix<double, 1, 1>::Identity(), gen)(0);
    }
  ...
  const Eigen::VectorXcd wind_freq = A.cwiseProduct(noise_freq);
...
void PhysicsSimulator::Run(
    ...
    std::unordered_map<std::string, std::shared_ptr<QuadStatePublisherNode>>
        quad_state_publishers,
    unsigned int seed) {
  // System time
  ...
  const auto x_wind_samples = GenerateWindAccelerationVector(
      this->options_.max_time, this->options_.max_frequency,
      this->options_.sigma_u_x, this->options_.L_u_x, this->options_.V, gen);

  const auto y_wind_samples = GenerateWindAccelerationVector(
      this->options_.max_time, this->options_.max_frequency,
      this->options_.sigma_u_y, this->options_.L_u_y, this->options_.V, gen);

  const auto z_wind_samples = GenerateWindAccelerationVector(
      this->options_.max_time, this->options_.max_frequency,
      this->options_.sigma_u_z, this->options_.L_u_z, this->options_.V, gen);
  ...
  for (size_t wind_idx = 0; wind_idx < wind_vector_size; ++wind_idx) {
    ...
    wind_instance.acceleration = Eigen::Vector3d(
        x_wind_samples.second[wind_idx], y_wind_samples.second[wind_idx],
  ...
  std::map<std::string, Eigen::Matrix<double, 9, 1>, std::less<std::string>,
           Eigen::aligned_allocator<
               std::pair<const std::string, Eigen::Matrix<double, 9, 1>>>>
      pva_perturbed_register;

  // Initially, the quads are in their current state
  for (const std::string& quad_name : quad_names) {
    pva_perturbed_register[quad_name] =
        (Eigen::Matrix<double, 9, 1>()
             << this->options_.initial_quad_positions[quad_name](0),
         this->options_.initial_quad_positions[quad_name](1),
         this->options_.initial_quad_positions[quad_name](2), 0, 0, 0, 0, 0, 0)
            .finished();
  }
  ...
  while (this->ok_) {
    ...
    const double end_time_float =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            current_time.time_since_epoch() + this->options_.simulation_time)
            .count();
    ...
    for (const std::string& quad_name : quad_names) {
      ...
      if (trajectory_size == 0) {
        // std::cout << "PhysicsSimulator::Run -- Empty Trajectory. Freezing."
        // << std::endl;
        continue;
      }
      ...
      Eigen::Matrix<double, 9, 1> pva_perturbed =
          pva_perturbed_register[quad_name];
      TimeSpan ts(0, 1, 0.5);
      while (true) {
        ...
        A.block(3, 0, 3, 3) =
            Eigen::Matrix<double, 3, 3>::Identity() * 1 * this->options_.kp;
        A.block(3, 3, 3, 3) =
            Eigen::Matrix<double, 3, 3>::Identity() * 1 * this->options_.kd;
        ...
        auto DynamicsFunction =
            [&](double time,
              ...
              for (size_t wind_idx = 0; wind_idx < wind_vector_size;
                   ++wind_idx) {
              ...
              Eigen::Matrix<double, 9, 1> t1 =
                  A * pva_intermediate + B1 * pva_intended;
              t1.block(3, 0, 3, 1) =
                  t1.block(3, 0, 3, 1)
        ...
        pva_perturbed =
            rk4.ForwardIntegrate(DynamicsFunction, pva_perturbed, ts);
    ...
    for (const auto& kv : quad_state_publishers) {
      ...
      const Eigen::Matrix<double, 9, 1> pva_perturbed =
          pva_perturbed_register[quad_name];
      QuadState quad_state((Eigen::Matrix<double, 13, 1>() << pva_perturbed(0),
                            pva_perturbed(1), pva_perturbed(2),
                            pva_perturbed(3), pva_perturbed(4),
                            pva_perturbed(5), 1, 0, 0, 0, 0, 0, 0)
                               .finished());
...
void PhysicsSimulator::Stop() { this->ok_ = false; }
}  // namespace game_engine
