...
Polygon Polygon::Expand(double dist) const {
  ...
  for (const Line2D& edge : this->edges_) {
    ...
    const Point2D center_to_start(dist * ((dx_start > 0) - (0 > dx_start)),
                                  dist * ((dy_start > 0) - (0 > dy_start)));

    const double dx_end = edge.End().x() - avg_x;
    const double dy_end = edge.End().y() - avg_y;
    const Point2D center_to_end(dist * ((dx_end > 0) - (0 > dx_end)),
...
Polygon Polygon::Shrink(double dist) const { return this->Expand(-dist); }
...
void Polygon::ConvexHullFromPoints(const std::vector<Point2D>& points) {
  ...
  while (true) {
    ...
    if (left_most_point == vertices.back()) {
      break;
    }
  }
...
Polygon::Polygon(const std::vector<Line2D>& edges) : edges_(edges) {
  this->UpdateVertices();
}
...
const std::vector<Line2D>& Polygon::Edges() const { return this->edges_; }
...
bool Polygon::Contains(const Point2D& point) const {
  for (const Line2D& edge : this->edges_) {
    if (false == edge.OnLeftSide(point)) {
      return false;
    }
  }
...
Polygon Polygon::BoundingBox() const {
  ...
  return Polygon({Line2D(Point2D(min_x, min_y), Point2D(max_x, min_y)),
