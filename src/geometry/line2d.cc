...
const Point2D& Line2D::Start() const { return this->start_; }

const Point2D& Line2D::End() const { return this->end_; }
...
Vec2D Line2D::AsVector() const { return this->end_ - this->start_; }

Vec2D Line2D::AsUnitVector() const { return this->AsVector().normalized(); }
...
Point2D Line2D::IntersectionPoint(
    const std::pair<Point2D, double>& other_sf) const {
  const std::pair<Point2D, double> this_sf = this->StandardForm();
  return (Eigen::Matrix<double, 2, 2>() << this_sf.first.transpose(),
...
Point2D Line2D::NormalIntersectionPoint(const Point2D point) const {
  ...
  return (Eigen::Matrix<double, 2, 2>() << sf.first.transpose(),
          A_prime.transpose())
             .finished()
             .inverse() *
...
bool Line2D::Contains(const Point2D& point) const {
  return std::abs(
             (point - this->start_).normalized().dot(this->AsUnitVector()) -
             1) < 1e-3 &&
         std::abs((point - this->end_).normalized().dot(-this->AsUnitVector()) -
                  1) < 1e-3;
}

bool Line2D::ProjectedContains(const Point2D& point) const {
  const Point2D projected_point =
      (point - this->start_).dot(this->AsUnitVector()) * this->AsUnitVector() +
      this->start_;
  return this->Contains(projected_point);
}

Point2D Line2D::Midpoint() const { return this->start_ + this->AsVector() / 2; }
}  // namespace game_engine
