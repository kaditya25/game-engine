...
int main(int argc, char** argv) {
  ...
  // Start ROS
  ros::init(argc, argv, "multi_quad_autonomy_protocol", ros::init_options::NoSigintHandler);
...
//  blue_balloon_position_publisher_node->Publish(setStartPositionBlue);
  ...
  std::thread ap_thread_multi_quad(
      [&]() {
        multi_quad_autonomy_protocol->Run(proposed_trajectory_clients, joy_mode, camera_mode);
      });
