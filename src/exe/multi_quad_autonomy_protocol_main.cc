...
#include "research-autonomy-protocols/multi_quad_autonomy_protocol.h"
...
int main(int argc, char** argv) {
  ...
  // Start ROS
  ros::init(argc, argv, "multi_quad_autonomy_protocol",
  ...
  if (false == nh.getParam("wind_intensity", wind_intensity_int)) {
    std::cerr << "Required parameter not found on server: wind_intensity"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  for (const auto& kv : quad_state_topics) {
    ...
    quad_state_subscribers.push_back(std::make_shared<QuadStateSubscriberNode>(
  ...
  BalloonStatus setStartStatusRed =
      *(red_balloon_status_subscriber_node->balloon_status_);
  ...
  std::shared_ptr<AutonomyProtocol> multi_quad_autonomy_protocol =
      std::make_shared<MultiQuadAutonomyProtocol>(
  ...
  std::thread ap_thread_multi_quad([&]() {
    multi_quad_autonomy_protocol->Run(proposed_trajectory_clients, joy_mode,
                                      camera_mode);
  });
  ...
  std::thread kill_thread([&]() {
    ...
    multi_quad_autonomy_protocol->Stop();
