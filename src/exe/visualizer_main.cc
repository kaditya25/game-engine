...
int main(int argc, char** argv) {
  ...
  if (false == nh.getParam("visualization_trajectory_topics",
                           visualization_trajectory_topics)) {
  ...
  std::unordered_map<std::string, std::shared_ptr<TrajectoryVisualizerNode>>
      trajectory_visualizers;
    ...
    for (const auto& kp : visualization_trajectory_topics) {
      if (kp.first == quad_name) {
        ...
        trajectory_visualizers[quad_name] =
            std::make_shared<TrajectoryVisualizerNode>(topic_subscribe,
                                                       topic_publish);
      }
    }
  }
  ...
  if (false == nh.getParam("quad_mesh_file_path", quad_mesh_file_path)) {
    std::cerr << "Required parameter not found on server: quad_mesh_file_path"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  if (false == nh.getParam("balloon_mesh_file_path", balloon_mesh_file_path)) {
    std::cerr
        << "Required parameter not found on server: balloon_mesh_file_path"
        << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  if (false == nh.getParam("goal_mesh_file_path", goal_mesh_file_path)) {
    std::cerr << "Required parameter not found on server: goal_mesh_file_path"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  const Eigen::Vector3d blue_balloon_position(blue_balloon_position_vector[0],
  ...
  const Eigen::Vector3d red_balloon_position(red_balloon_position_vector[0],
  ...
  const Eigen::Vector3d goal_position(goal_position_vector[0],
  ...
  if (false == nh.getParam("team_assignments", team_assignments)) {
    std::cerr << "Required parameter not found on server: team_assignments"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  for (const auto& kv : team_assignments) {
    ...
    const std::string& quad_name = kv.first;
    quad_view_options.quads.push_back(std::make_pair<>(
        std::make_pair<>(color, quad_name), quad_state_warden));
  }

  ViewManager::BalloonViewOptions balloon_view_options;
  balloon_view_options.balloon_mesh_file_path = balloon_mesh_file_path;
  balloon_view_options.balloons.push_back(
      std::make_pair("red", red_balloon_position));
  balloon_view_options.balloons.push_back(
      std::make_pair("blue", blue_balloon_position));
  ...
  for (const auto& kv : team_assignments) {
    const std::string& quad_name = kv.first;
    trajectory_view_options.trajectories.push_back(
        std::make_pair<>(quad_name, trajectory_visualizers[quad_name]));
  }

  auto view_manager = std::make_shared<ViewManager>();
  std::thread view_manager_thread([&]() {
    view_manager->Run(quad_view_options, balloon_view_options,
                      goal_view_options, environment_view_options,
