...
#include "research-autonomy-protocols/manual_control_protocol.h"
...
int main(int argc, char** argv) {
  ...
  for (const auto& kv : proposed_trajectory_topics) {
    const std::string& quad_name = kv.first;
    const std::string& topic = kv.second;
    if (std::find(blue_quad_names.begin(), blue_quad_names.end(), quad_name) !=
        blue_quad_names.end()) {
  ...
  std::shared_ptr<AutonomyProtocol> manual_control_protocol =
      std::make_shared<ManualControlProtocol>(
