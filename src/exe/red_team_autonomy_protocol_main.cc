...
#include "research-autonomy-protocols/red_team_autonomy_protocol.h"
...
int main(int argc, char** argv) {
  ...
  // Start ROS
  ros::init(argc, argv, "red_team_autonomy_protocol",
  ...
  auto game_snapshot = std::make_shared<GameSnapshot>(
      red_quad_names, blue_quad_names, quad_state_warden,
  ...
  for (const auto& kv : proposed_trajectory_topics) {
    const std::string& quad_name = kv.first;
    const std::string& topic = kv.second;
    if (std::find(red_quad_names.begin(), red_quad_names.end(), quad_name) !=
        red_quad_names.end()) {
  ...
  std::shared_ptr<AutonomyProtocol> red_team_autonomy_protocol =
      std::make_shared<RedTeamAutonomyProtocol>(
          red_quad_names, blue_quad_names, game_snapshot,
  ...
  std::thread ap_thread_red_team([&]() {
    red_team_autonomy_protocol->Run(proposed_trajectory_clients, joy_mode,
                                    camera_mode);
  });
