...
TrajectoryClientNode::TrajectoryClientNode(const std::string& topic) {
  this->client_ = node_handle_.serviceClient<mg_msgs::PVAYT>(topic);
}
...
TrajectoryCode TrajectoryClientNode::Request(const Trajectory& trajectory) {
  ...
  if (client_.call(srv)) {
    // ROS_INFO("Client: Successfully called service. Response: %d ",
    // srv.response.status);
