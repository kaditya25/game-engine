...
namespace game_engine {
  TrajectoryVisualizerNode::TrajectoryVisualizerNode(const std::string& topic_subscribe,
                                                     const std::string& topic_publish) {
    this->node_handle_ = ros::NodeHandle("/game_engine/");
    this->subscriber_ = node_handle_.subscribe(topic_subscribe, 
                                               1, 
                                               &TrajectoryVisualizerNode::SubscriberCallback, 
                                               this);
    this->publisher_guard_ = std::make_shared<PublisherGuard<nav_msgs::Path>>(topic_publish);
  }

  void TrajectoryVisualizerNode::SubscriberCallback(const mg_msgs::PVAYStampedTrajectory& msg) {
