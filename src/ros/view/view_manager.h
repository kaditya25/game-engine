...
class ViewManager {
 public:
  ...
  struct QuadViewOptions {
    ...
    std::vector<std::pair<std::pair<std::string, std::string>,
                          std::shared_ptr<QuadStateWarden>>>
        quads;
  ...
  struct EnvironmentViewOptions {
    Plane3DView::Options ground_view_options{"world", 0.0f, 1.0f, 0.0f, 1.0f};
    Plane3DView::Options wall_view_options{"world", 0.0f, 0.0f, 0.0f, 0.1f};
    PolyhedronView::Options obstacle_view_options{"world", 1.0f, 1.0f, 1.0f,
                                                  1.0f};
    Map3D map;
  ...
  struct TrajectoryViewOptions {
    std::vector<
        std::pair<std::string, std::shared_ptr<TrajectoryVisualizerNode>>>
        trajectories;
 ...
 private:
  void RunQuadPublisher(const QuadViewOptions quad_view_options);
  void RunBalloonPublisher(const BalloonViewOptions balloon_view_options);
  void RunGoalPublisher(const GoalViewOptions goal_view_options);
