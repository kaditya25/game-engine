...
class Graph {
 private:
  ...
  std::unordered_map<std::shared_ptr<T>, std::vector<DirectedEdge<T>>,
                     class T::HashPointer, class T::EqualsPointer>
      edge_graph_;
 ...
 public:
  ...
  // given node
  const std::vector<std::shared_ptr<T>> Neighbors(
      const std::shared_ptr<T>& node) const;
...
inline void Graph<T>::AddEdges(const std::vector<DirectedEdge<T>>& edges) {
  std::for_each(edges.begin(), edges.end(),
                [this](const DirectedEdge<T>& edge) { this->AddEdge(edge); });
}

template <class T>
inline const std::vector<DirectedEdge<T>> Graph<T>::Edges(
    const std::shared_ptr<T>& node) const {
  try {
...
template <class T>
inline const std::vector<std::shared_ptr<T>> Graph<T>::Neighbors(
    const std::shared_ptr<T>& node) const {
