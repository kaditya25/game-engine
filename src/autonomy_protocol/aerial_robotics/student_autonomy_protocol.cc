...
game_engine::TrajectoryVector3D RunAstarPlusNumWaypointPolySolver(
    const Graph3D &graph, const std::shared_ptr<game_engine::Node3D> start_ptr,
    const std::shared_ptr<game_engine::Node3D> end_ptr, int derivative_order,
    int seg) {
  // A star algo
  ...
  solver_options.derivative_order =
      derivative_order;  // Minimize something (2 for accel)
  ...
  for (int t_idx = 0; t_idx < ppp.cols() + 1; t_idx++) {
    double flight_time = ppp.row(0)(t_idx);
    double x = ppp.row(1)(t_idx);
    double y = ppp.row(2)(t_idx);
    double z = ppp.row(3)(t_idx);
    double vx = vvv.row(1)(t_idx);
    double vy = vvv.row(2)(t_idx);
    double vz = vvv.row(3)(t_idx);
    double ax = aaa.row(1)(t_idx);
    double ay = aaa.row(2)(t_idx);
    double az = aaa.row(3)(t_idx);
    double yaw = 0;
...
StudentAutonomyProtocol::UpdateTrajectories() {
  ...
  std::shared_ptr<Node3D> current_ptr = std::make_shared<Node3D>(current_pos);
  ...
  trajectory_vector = RunAstarPlusNumWaypointPolySolver(
      graph_of_arena, current_ptr, end_ptr, 2, seg);
