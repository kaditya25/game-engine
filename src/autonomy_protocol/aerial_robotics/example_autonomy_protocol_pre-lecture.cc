...
ExampleAutonomyProtocol::UpdateTrajectories() {
  ...
  if (first_time) {
    // TODO: Perform first-time initialization
  }

  // Obtain current balloon positions and popped states
  // TODO: Obtain positions and states of red and blue balloons
  ...
  if (halt ||
    ...
    for (size_t idx = 0; idx < 20; ++idx) {
      ...
      // TODO: Push back stationary points to form the "halt" trajectory
    }
    // TODO: Return the "halt" trajectory
  }
  ...
  // function returns type TrajectoryCode, which contains three values:
  //
  // (1) The mediation layer code that specifies success or specific failure
  // types.
  //
  // (2) The failure value (e.g., if the velocity limit is 4.0 m/s and you
  // submit 5.0 m/s, the value returned is 5.0).
  //
  // (3) The failure index. This is the trajectory sample index that caused the
  // mediation layer to kick back an error code.  The index is the sampled index
  // (specified from the P4 sampling process).  You can figure out the waypoint
  // index by a simple math transformation.

  // TODO: submit trajectory to prevetter and get response
