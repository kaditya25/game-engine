...
#include "red_team_autonomy_protocol.h"
...
namespace game_engine {
  // The red team autonomy protocol attempts to pop a blue balloon while defending 
  // its own red balloon. The protocol switches between offense and defense
  ...
  TrajectoryVector3D RedTeamAutonomyProtocol::targetToTrajectory(Eigen::Vector3d current_position, Eigen::Vector3d current_velocity, 
  ...
  std::unordered_map<std::string, Trajectory> RedTeamAutonomyProtocol::UpdateTrajectories() 
  {
    ...
    { 
      ...
      if((blue_balloon_pos - current_pos[ii]).norm() < (red_balloon_pos - current_pos[ii]).norm()) 
      {
        bool offense = true;
      }
      else
      ...
      if(offense==true) //play offense
      {
        ...
        Eigen::Vector3d vec_to_balloon = (blue_balloon_pos - current_pos[ii])
          /(blue_balloon_pos - current_pos[ii]).norm();
        ...
        else
        {
          ...
          {
            //Target the blue balloon
            target_pos[ii] = (blue_balloon_pos - current_pos[ii])/(blue_balloon_pos - current_pos[ii]).norm()
              + current_pos[ii];
          }
        }
      }
      ...
      else //play defense
      {
        ...
        else
        {
          ...
          Eigen::Vector3d enemy_vec_to_balloon = red_balloon_pos - enemy_current_pos[ii];
